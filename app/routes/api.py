...
async def chat_completions(
    ...
    """OpenAI-compatible chat completions endpoint with custom generation parameters.
    Supports system, user, and assistant roles with proper Gemma formatting.
    """
    start_time = time.time()
    ...
    try:
        ...
        # Check quota (same as your existing endpoints)
        api_key = next(key for key, value in settings.API_KEYS.items() if value['name'] == user_info['name'])
        remaining = settings.MAX_DAILY_REQUESTS - user_quotas.get(api_key, {}).get("count", 0)
